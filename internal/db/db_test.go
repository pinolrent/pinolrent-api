...
func TestSeedAdminCreateAndUpdate(t *testing.T) {
	...
	var hash string
	if err := d.QueryRow(`SELECT password_hash FROM users WHERE email = 'admin@example.com'`).Scan(&hash); err != nil {
...
func TestSeedAdminCreateIdempotentCount(t *testing.T) {
	...
	var n int
	if err := d.QueryRow(`SELECT COUNT(*) FROM users WHERE email = 'admin@example.com'`).Scan(&n); err != nil {
...
func TestSeedAdminConflict(t *testing.T) {
	...
	if _, err := d.Exec(`INSERT INTO users (email, password_hash, role) VALUES ('client@example.com', 'h', 'client')`); err != nil {
